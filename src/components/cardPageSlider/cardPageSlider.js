    ...
    const showCardImages = (selector) => {
        ...
        getData(`http://localhost:3000/${selector}`)
            .then(data => {
                ...
                const thumbsContainer = document.querySelector('.cardPageSlider__thumbs');
                ...
                data.forEach(({ src, alt }) => {
                    ...
                    slide.classList.add('swiper-slide', 'cardPageSlider__img');
                    slide.innerHTML = `<img src="${src}" alt="${alt}">`;
                    wrapper.appendChild(slide);

                    const thumb = document.createElement('img');
                    thumb.classList.add('cardPageSlider__thumbs-thumb');
                    thumb.src = src;
                    thumb.alt = alt;
                ...
                const thumbs = document.querySelectorAll('.cardPageSlider__thumbs-thumb');
