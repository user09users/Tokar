    ...
    if (
        ...
        getData("http://localhost:3000/cards")
            .then(data => {
                ...
                // Intro
                const introBlocks = document.querySelectorAll(".itemDescr__intro-block");
                ...
                // Description
                const descrWrapper = document.querySelector(".itemDescr__descr-ul");
                card.description.forEach(({ label, value }) => {
                    const li = document.createElement("li");
                    li.classList.add("itemDescr__descr-item");
                    li.innerHTML = `<div>${label}</div><span>${value}</span>`;
                    descrWrapper.appendChild(li);
                });

                // Features
                const featuresWrapper = document.querySelector(".itemDescr__images-ul");
                card.features.forEach(({ icon, alt, text }) => {
                    const li = document.createElement("li");
                    li.classList.add("itemDescr__images-item");
